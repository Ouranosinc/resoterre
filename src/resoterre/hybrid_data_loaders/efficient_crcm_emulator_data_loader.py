...
class CRCMEmulatorDataset(td.Dataset):  # type: ignore[misc]
    ...
    def __init__(
        ...
        for simulation in simulations:
            ...
            xarray_dataset_gcm = xarray.open_dataset(zarr_directories[0], engine="zarr", chunks={}) # chunks={} ensures data is not loaded into memory all at once
            ...
            xarray_dataset_crcm = xarray.open_dataset(zarr_directories[0], engine="zarr", chunks={}) 
    ...
    def get_open_dataset(self, dataset_type: str, key: str) -> xarray.Dataset:
        ...
        if dataset_type == "gcm":
            if key not in self.gcm_open_dataset:
                ...
                self.gcm_open_dataset[key] = xarray.open_mfdataset(self.gcm_zarr[key], engine="zarr", chunks={}, combine="by_coords", parallel=True) 
            ...
            if key not in self.crcm_open_dataset:
                ...
                self.crcm_open_dataset[key] = xarray.open_mfdataset(self.crcm_zarr[key], engine="zarr", chunks={}, combine="by_coords", parallel=True) 
    ...
    def __getitem__(self, idx: int) -> dict[str, np.ndarray]:
        ...
        for i, variable_name in enumerate(self.gcm_variables):
            ...
            gcm_data = xarray_variable.isel(time=gcm_idx).load().values
