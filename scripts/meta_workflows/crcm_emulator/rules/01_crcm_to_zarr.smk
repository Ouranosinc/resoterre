...
# ToDo: get_chunk_indices is a calendar_utils candidate?
...
def expected_manifests(wildcards):
    ...
    for simulation in config_obj.preprocessing_simulations:
        ...
        chunk_indices = get_chunk_indices(
            gcm=simulation[0],
            start_datetime=config_obj.crcm_preprocessing_start_datetime,
        ...
        for chunk_index_tuple in chunk_indices:
            ...
            for variable_name in config_obj.crcm_preprocessing_variables:
                full_manifest = f"manifests/crcm_to_zarr_{simulation_str}_{variable_name}_{chunk_str}.done"
                list_of_expected_manifests.append(full_manifest)
    init_manifest = f"manifests/crcm_to_zarr_{init_gcm_str}_{init_variable}_0_{chunk_size * chunks_per_task - 1}.done"
...
rule crcm_to_zarr:
    ...
    output:
        touch("manifests/crcm_to_zarr_{gcm}_{pathway}_{realization}_{variable_name}_{i_start}_{i_end}.done")
    params:
        path_script=Path(snakefile_dir, "01_crcm_to_zarr.py"),
        workflow_dir=workflow_dir,
        config_yaml=config["config_yaml"],
