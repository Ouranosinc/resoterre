"""Snakemake workflow for CRCM emulator U-Net training.

To run this workflow, use the command:
snakemake -s 03_crcm_emulator_training.smk -j1 --config config_yaml=config.yaml --directory=/workflow_directory
"""
...
def expected_manifests(wildcards):
    ...
    for epoch in range(nb_of_epochs):
        list_of_expected_manifests.append(
            f"manifests/crcm_emulator_training_epoch_{epoch + 1}.done")
...
rule first_epoch:
    input:
        upstream_input
    output:
        touch("manifests/crcm_emulator_training_epoch_1.done")
    params:
        path_script=Path(snakefile_dir, "03_crcm_emulator_training.py"),
...
rule train_epoch:
    input:
        lambda wc: f"manifests/crcm_emulator_training_epoch_{int(wc.epoch) - 1}.done"
    output:
        touch("manifests/crcm_emulator_training_epoch_{epoch}.done")
    params:
        path_script=Path(snakefile_dir, "03_crcm_emulator_training.py"),
        workflow_dir=workflow_dir,
        config_yaml=config["config_yaml"],
