...
from resoterre.datasets.hrdps.hrdps_integrity_check import hrdps_integrity_check_datetime_list
...
def hrdps_integrity_check_4smk(output_file: Path | str, period_str: str, config: RDPSToHRDPSOnDiskConfig) -> None:
    """
    ...
    if config.path_hrdps is None:
        raise ValueError("path_hrdps must be provided in the config for HRDPS integrity check")
    manifest = hrdps_integrity_check_datetime_list(
        config.path_hrdps,
        config.hrdps_variables,
        config.forecast_hours,
        list_of_datetime=list_of_datetime,
        source_type="caspar_012",
    )  # Other source types are not supported for now
...
def main() -> None:
    """Main function to run the HRDPS integrity check in a Snakemake workflow."""
    ...
    try:
        hrdps_integrity_check_4smk(snakemake.output[0], period_str, snakemake.params.config_obj)
    except Exception:
